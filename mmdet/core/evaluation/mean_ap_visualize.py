...
novel_sets = [['bird', 'bus', 'cow', 'motorbike', 'sofa1'],
...
def discretize(recall, precision):
    step = 1 / 1000
    recall_dis = np.linspace(0, 1, 1001)
    precision_dis = np.zeros(1001, dtype=np.float)
    iter_idx = 0
    for i in range(recall_dis.shape[0]):
        if i==0:
            continue
        low = recall_dis[i] - step
        high = recall_dis[i] + step
        # print('low: ', low)
        # print('high: ', high)
        while iter_idx < recall.shape[0] and recall[iter_idx] < low:
            iter_idx += 1
        pre_sum = precision[0]*0
        pre_count = 0
        while iter_idx < recall.shape[0] and recall[iter_idx] <= high:
            pre_sum += precision[iter_idx]
            pre_count += 1
            iter_idx += 1
        # print(pre_sum, pre_count)
        precision_dis[i] = pre_sum / (pre_count+1e-4)
        if iter_idx >= recall.shape[0]:
            break
    return recall_dis, precision_dis
...
def map_roc_pr(det_results,
    ...
    for i in range(num_classes):
        ...
        f_measure = np.mean(2*(top/(down+1e-6)))
    ...
    print('################################')
    for i in range(num_classes):
        if label_names[i] not in novel_sets[novel_id]:
            continue
        # plt.subplots_adjust(hspace=0.3)
        subplot_id = extra_info['subplot_id']
        plt.subplot(subplot_id)
        # plt.plot(recall_list[i], precision_list[i], linewidth=2, \
        #          label='{} (AP = {:.2f})'.format(label_names[i], ap_list[i]))
        plt.plot(recall_list[i], precision_list[i], linewidth=2, \
                 label='{}'.format(label_names[i]))
        # plt.title('Precision-Recall')
        if extra_info is not None:
            title_text = extra_info['title']
        else:
        ...
        plt.axis([0, 1, 0, 1])
        plt.legend(markerscale=0.3, fontsize=fontsize, labelspacing=0.02)
    ...
    # model_label = 'None'
    # if extra_info is not None:
    #     model_label = extra_info['model_label']
    #
    # recall_sum = np.zeros(1001,)
    # precision_sum = np.zeros(1001,)
    # for i in range(num_classes):
    #     recall_dis, precision_dis = discretize(recall_list[i], precision_list[i])
    #     print(precision_dis.shape)
    #     print(precision_dis)
    #     if label_names[i] in novel_sets[novel_id]:
    #         if recall_sum.shape[0] < recall_dis.shape[0]:
    #             z = np.zeros(recall_dis.shape[0]-recall_sum.shape[0],)
    #             recall_sum = np.concatenate((recall_sum, z), axis=0)
    #             precision_sum = np.concatenate((precision_sum, z), axis=0)
    #         if recall_sum.shape[0] > recall_dis.shape[0]:
    #             z = np.zeros(recall_sum.shape[0]-recall_dis.shape[0],)
    #             recall_dis = np.concatenate((recall_dis, z), axis=0)
    #             precision_dis = np.concatenate((precision_dis, z), axis=0)
    #         recall_sum = recall_sum + recall_dis
    #         precision_sum = precision_sum + precision_dis
    # recall_mean = recall_sum / 5
    # precision_mean = precision_sum / 5
    #
    # plt.plot(recall_mean, precision_mean, linewidth=2, \
    #          label='{}'.format(model_label))
    # plt.show()

    if extra_info['subplot_id']==133:
        plt.show()
    print('################################')
    # plt.show()
    # plt.savefig('/home/luyue/Documents/mmdetection_old/PR_Curve_each_class.png')
