...
class DetachFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if (self.grad_scale is not None) and self.training:
            inputs = [scale_tensor_gard(i, self.grad_scale) for i in inputs]
        # build laterals
        laterals = [
            lateral_conv(inputs[i + self.start_level].detach())
            for i, lateral_conv in enumerate(self.lateral_convs[:-1])
        ]

        laterals.append(self.lateral_convs[-1](inputs[len(self.lateral_convs) - 1 + self.start_level]))
