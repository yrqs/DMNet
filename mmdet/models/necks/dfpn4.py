...
class DFPN4(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.save_outs:
            ...
            attentions_reg = []
        for out in outs:
            att_cls = self.cls_att_conv(out)
            out_cls = out * att_cls.sigmoid()
            outs_cls.append(out_cls)

            att_reg = self.reg_att_conv(out)
            out_reg = out * att_reg.sigmoid()
            outs_reg.append(out_reg)
            if self.save_outs:
                attentions_cls.append(att_cls)
                attentions_reg.append(att_reg)
        ...
        if self.save_outs:
            ...
            res['fpn_outs'] = outs
            res['attentions_cls'] = attentions_cls
            res['attentions_reg'] = attentions_reg
            res['outs_cls'] = outs_cls
            res['outs_reg'] = outs_reg
            save_idx = 1
            save_path_base = 'mytest/dfpn4_outs.pth'
