...
class FSBBoxHead2(nn.Module):
    ...
    def __init__(self,
        ...
        super(FSBBoxHead2, self).__init__()
        ...
        self.neg_score_weight = neg_score_weight
        if neg_cls:
            fc_cls_channels = (len(base_ids) + len(novel_ids) + 1) if base_ids is not None else num_classes
    ...
    def forward(self, x):
        ...
        if self.neg_cls:
            cls_score_neg = self.fc_cls_neg(x)
            if self.base_ids is not None:
                out_channels = [0]
                for id in self.base_ids:
                    out_channels.append(id+1)
            ...
            extra = cls_score_neg
            cls_score[:, 1:] = cls_score[:, 1:] - self.neg_score_weight * cls_score_neg[:, 1:]
            # cls_score = torch.cat([cls_score, cls_score_neg], dim=1)
        # cls_score = torch.zeros_like(cls_score)
        # cls_score[:, 1] = 5.
        # bbox_pred = torch.zeros_like(bbox_pred)
        if self.training:
            return cls_score, bbox_pred, extra
        else:
    ...
    def loss(self,
        ...
        losses = dict()
        # print('(labels > 0).sum()1: ', (labels > 0).sum())
        # print('(labels > 0)1: ', (labels > 0).sum())
        ...
        if self.neg_cls:
            ...
            labels[labels < 0] = 0
        # print('(labels > 0).sum()2: ', (labels > 0).sum())
        # print('(labels > 0)2: ', (labels > 0).sum())
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if self.neg_cls:
                    losses['loss_cls_neg'] = self.loss_cls(
                        cls_score_neg,
                        neg_labels,
                        label_weights,
                        avg_factor=avg_factor,
                        reduction_override=reduction_override)
                    losses['acc_neg'] = accuracy(cls_score_neg, neg_labels)
    ...
    def get_det_bboxes(self,
        ...
        # if self.neg_cls:
        #     cls_score = cls_score[:, :self.num_classes]
