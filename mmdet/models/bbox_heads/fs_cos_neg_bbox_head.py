...
class FSCosNegBBoxHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(FSCosNegBBoxHead, self).__init__()
        ...
        self.fc_cls_neg = nn.Linear(in_channels, fc_cls_channels, bias=False)
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.fc_cls_neg.weight, 0, 0.01)
    ...
    def forward(self, x):
        ...
        if self.use_cos:
            ...
            # neg_offset = self.neg_offset_fc(fg_w_norm).view(fg_w_norm.size(0), self.neg_num_modes, fg_w_norm.size(-1))
            ...
            fg_cls_score = cos_sim * self.cos_scale
        ...
        if self.training:
            return cls_score, bbox_pred, (cos_sim, cos_sim_neg)
        else:
            return cls_score, bbox_pred
    ...
    def loss(self,
        ...
        if self.use_cos:
            ...
            if (self.add_cos_center_loss or self.add_soft_cos_center_loss):
                ...
                cos_sim_pos = cos_sim[pos_inds][labels_pos_one_hot]
            ...
            if self.triplet_margin is not None:
                ...
                with torch.no_grad():
                    cls_score_fg_ori = cos_sim.detach().clone()
                    labels_one_hot = F.one_hot(labels.flatten(0), num_classes=self.num_classes).byte()[:, 1:]
                    cls_score_fg_ori[labels_one_hot] = -1.
                    max_val, neg_labels = cls_score_fg_ori.max(dim=1)
                    neg_labels = neg_labels + 1
                    neg_labels[max_val < self.neg_thresh] = 0
                cos_sim_cat = torch.cat([cos_sim[:, :, None], cos_sim_neg[:, :, None]], dim=-1)
                cos_sim_cat_pos = cos_sim_cat[labels_one_hot]
                losses['loss_hn_pos'] = F.relu(cos_sim_cat_pos[:, 1] - cos_sim_cat_pos[:, 0] + self.neg_margin).mean() * self.loss_hn_pos_weight
                if (neg_labels > 0).sum() == 0:
                    ...
                    cos_sim_cat_neg = cos_sim_cat[neg_labels_one_hot]
                    losses['loss_hn_neg'] = F.relu(cos_sim_cat_neg[:, 0] - cos_sim_cat_neg[:, 1] + self.neg_margin).mean() * self.loss_hn_neg_weight
