...
class DMLHead(nn.Module):
    ...
    def forward(self, x, save_outs=False):
        ...
        distances = torch.sqrt(((emb_vectors_ex.epxand_as(reps_ex) - reps_ex)**2).sum(-1))
        ...
        distances_neg = torch.sqrt(((emb_vectors_ex.epxand_as(neg_reps) - neg_reps)**2).sum(-1)).permute(0, 3, 4, 1, 2).contiguous()
...
class DMLNegFCBBoxHead2(BBoxHead):
    ...
    def loss(self,
        ...
        loss_emb_pos, loss_emb_neg = self.loss_emb_hard(distance, distance)
        return losses
    ...
    def get_neg_labels_single(self, cls_score, labels):
        ...
        cls_score_other[labels_one_hot][labels_one_hot] = 0.
