...
class DMLHead(nn.Module):
    ...
    def forward(self, x, save_outs=False):
        ...
        reps_ex = reps[None, :, :, :].expand(emb_vectors.size(0), -1, -1, -1)
        emb_vectors_ex = emb_vectors[:, None, None, :]

        distances = torch.sqrt(((emb_vectors_ex.expand_as(reps_ex) - reps_ex)**2).sum(-1))
        ...
        neg_reps = F.normalize(neg_reps, p=2, dim=2)[None, :, :, :].expand(emb_vectors.size(0), -1, -1, -1)

        distances_neg = torch.sqrt(((emb_vectors_ex.expand_as(neg_reps) - neg_reps)**2).sum(-1))

        probs = torch.exp(-(distances + self.beta*torch.sub(2, distances_neg.min(dim=2, keepdim=True)[0]))**2/(2.0*self.sigma**2))
        if self.cls_norm:
...
class DMLNegFCBBoxHead(BBoxHead):
    ...
    def forward(self, x):
        ...
        cls_score, distance, distances_neg, probs_ori = self.cls_head(x_cls) if self.with_cls else None
        bbox_pred = self.fc_reg(x_reg) if self.with_reg else None
        if self.training:
            return cls_score, bbox_pred, distance, distances_neg, probs_ori
        else:
            return cls_score, bbox_pred

    @force_fp32(apply_to=('cls_score', 'bbox_pred', 'distance', 'distance_neg', 'prob_ori'))
    def loss(self,
             cls_score,
             bbox_pred,
             distance,
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = self.loss_bbox(
                    ...
                    reduction_override=reduction_override)
        losses['emb'] = self.loss_emb(distance, labels, label_weights, self.num_classes)
        neg_labels = self.get_neg_labels_single(cls_score, labels)
        loss_emb_pos, loss_emb_neg = self.loss_emb_hard(distance, distance_neg, labels, neg_labels, label_weights)
        losses['loss_emb_pos'] = loss_emb_pos
        losses['loss_emb_neg'] = loss_emb_neg
        return losses

    def loss_emb(self, distance, labels, label_weights, num_classes):
        pos_inds = labels > 0
        if pos_inds.sum() == 0:
        ...
        labels_pos_one_hot = F.one_hot(labels_pos.flatten(0), num_classes=num_classes).byte()[:, 1:]
    ...
    def get_neg_labels_single(self, cls_score, labels):
        labels_one_hot = F.one_hot(labels.flatten(0), num_classes=self.num_classes).byte()[:, 1:]
        cls_score_other = cls_score.detach().clone()[:, 1:]
        cls_score_other[labels_one_hot] = 0.
        pred_cls_score, pred_cls_id = cls_score_other.max(dim=-1)
        ...
        # neg_labels_one_hot = F.one_hot(neg_labels.flatten(0), num_classes=self.num_classes).byte()[:, 1:]
        return neg_labels

    def loss_emb_hard(self, distance, distance_neg, labels, labels_neg, label_weights):
        distance_neg = distance_neg.min(dim=-1, keepdim=True)[0]

        distance_cat = torch.cat([distance, distance_neg], dim=-1)

        labels_ont_hot = F.one_hot(labels.flatten(0), num_classes=self.num_classes)[:, 1:]
        distance_cat_pos = distance_cat[labels_ont_hot==1].unsqueeze(-1)
        labels = labels.reshape(-1)
        pos_inds = labels > 0
        label_weights_pos = label_weights.reshape(-1)[pos_inds]
        labels_pos = distance_cat_pos.new_full((distance_cat_pos.shape[0], ), 1, dtype=torch.long)
        loss_emb_pos = self.loss_emb(distance_cat_pos, labels_pos, label_weights_pos, 3)

        neg_labels_one_hot = F.one_hot(labels_neg.flatten(0), num_classes=self.num_classes)[:, 1:]
        ...
        labels_neg = distance_cat_neg.new_full((distance_cat_neg.shape[0], ), 2, dtype=torch.long)
        loss_emb_neg = self.loss_emb(distance_cat_neg, labels_neg, label_weights_neg, 3)
