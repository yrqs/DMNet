...
class RetinaDMLNegHead6(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
    ...
    def loss(self,
        ...
        for i in range(len(cls_scores)):
            ...
            losses_emb_hn_neg.append(loss_emb_neg)
            losses_emb_hn_pos.append(loss_emb_pos)

        losses_emb_neg = []
        for i in range(len(distances_neg)):
            loss_emb_neg = self.loss_emb_single(
                distances_neg[i],
                neg_labels_list[i].reshape(distances_neg[0].size(0), -1),
                label_weights_list[i],
                num_total_samples=num_total_samples_emb_neg)
            losses_emb_neg.append(loss_emb_neg)

        return dict(loss_cls=losses_cls,
                    loss_bbox=losses_bbox,
                    loss_emb=losses_emb,
                    loss_emb_hn_neg=losses_emb_hn_neg,
                    loss_emb_hn_pos=losses_emb_hn_pos,
