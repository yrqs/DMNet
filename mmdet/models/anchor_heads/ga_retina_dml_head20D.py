...
class FeatureAdaptionCls(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(deformable_groups):
            conv_offset = nn.Conv2d(
                conv_offset_in_channels//deformable_groups + 1, offset_channels, conv_offset_kernel_size, padding=conv_offset_kernel_size//2,  bias=False)
            self.conv_offsets.append(conv_offset)
    ...
    def init_weights(self):
        for conv_offset in self.conv_offsets:
            normal_init(conv_offset, std=0.1)
        normal_init(self.conv_adaption, std=0.01)

    def forward(self, x, x_enhance, loc):
        offsets = []
        x_enhance_split = x_enhance.split(x_enhance.size(1)//self.deformable_groups, dim=1)
        for i, x_enhance_split_i in enumerate(x_enhance_split):
            offset = self.conv_offsets[i](torch.cat([x_enhance_split_i, loc.detach()], dim=1))
            offsets.append(offset)
        x = self.relu(self.conv_adaption(x, torch.cat(offsets, dim=1)))
        return x

@HEADS.register_module
class GARetinaDMLHead20D(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        self.sigma = sigma
        self.deformable_groups_cls = deformable_groups_cls
        super(GARetinaDMLHead20D, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.cls_feat_enhance = nn.Conv2d(self.feat_channels, self.deformable_groups_cls, 3, stride=1, padding=1)

        self.feature_adaption_cls = FeatureAdaptionCls(
            self.feat_channels,
            self.feat_channels,
            kernel_size=3,
            conv_offset_in_channels=self.deformable_groups_cls+1,
            conv_offset_kernel_size=3,
            deformable_groups=self.deformable_groups_cls)
    ...
    def forward_single(self, x):
        ...
        feat_cls = self.feature_adaption_cls(cls_feat, cls_feat_enhance_pre, loc_pred)
    ...
    def forward(self, feats):
        if self.training:
            ...
            if self.save_outs:
                ...
                res['cls_loc'] = tuple([cls_feat_enhance_pre[:, -1].unsqueeze(1) for cls_feat_enhance_pre in cls_feat_enhance_pres])
                res['cls_feat_enhance'] = tuple([cls_feat_enhance_pre[:, :-1] for cls_feat_enhance_pre in cls_feat_enhance_pres])
    ...
    def calculate_neg_weights(self, labels, label_weights, feature_size):
        pos_labels = labels.gt(0).view(labels.size(0), feature_size[0], feature_size[1], 1)
        if int(pos_labels.sum()) == 0:
            return label_weights
        pos_xy_list = []
        pos_num_list = []
        for batch_idx in range(pos_labels.size(0)):
            pos_num = int(pos_labels[batch_idx].sum())
            pos_xy = pos_labels[batch_idx].squeeze(-1).nonzero().float()
            pos_xy_list.append(pos_xy)
            pos_num_list.append(pos_num)

        xy = torch.ones(feature_size).to(labels.device).nonzero().float().view(feature_size+(2,)).unsqueeze(-2)

        weights_list = []
        for batch_idx in range(pos_labels.size(0)):
            if pos_num_list[batch_idx] == 0:
                weights = torch.ones(pos_labels.shape[1:-1] + (1, )).unsqueeze(0).to(labels.device)
                weights_list.append(weights)
                continue
            pos_xy = pos_xy_list[batch_idx].unsqueeze(0).unsqueeze(0).expand(xy.size(0), xy.size(1), -1, -1)
            distance = torch.sqrt(((pos_xy - xy)**2).sum(-1))
            weights = torch.exp(-distance**2/(2.0*1.0**2))
            weights = weights.sum(-1, keepdim=True)
            weights[pos_labels[batch_idx] > 0] = 0.0
            weights = weights / weights.max()
            weights[pos_labels[batch_idx] > 0] = 1.0
    ...
    def loss_single(self, cls_score, bbox_pred, labels, label_weights,
        ...
        label_weights = self.calculate_neg_weights(labels, label_weights, cls_score.shape[-2:]).detach()
