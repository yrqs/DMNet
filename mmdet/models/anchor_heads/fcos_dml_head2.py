...
class DMLHead(nn.Module):
    def __init__(self,
        ...
        self.sigma = sigma
        self.sigma_fg = sigma_fg
        self.emb_module = emb_module
        self.emb_channels = emb_channels
        self.cls_norm = cls_norm
        self.representations = nn.Embedding(self.class_num*self.num_modes, self.emb_channels[-1]).weight
    ...
    def forward(self, x, save_outs=False):
        ...
        reps_ori = self.representations.unsqueeze(0)
        ...
        reps_att = self.mlp(reps_att)
        reps_att = F.normalize(reps_att, p=2, dim=2).squeeze(0).unsqueeze(1)

        emb_vectors_norm = emb_vectors_norm.permute(0, 2, 3, 1).unsqueeze(3).unsqueeze(4)
        emb_vectors_norm = emb_vectors_norm.expand(-1, -1, -1, self.class_num, self.num_modes, -1)
        distances = torch.sqrt(((emb_vectors_norm - reps_att)**2).sum(-1)).permute(0, 3, 4, 1, 2).contiguous()
        ...
        if save_outs:
            return cls_score, distances, emb_vectors, reps_att
...
class FCOSDMLHead2(nn.Module):
    ...
    def _init_layers(self):
        ...
        self.fcos_cls = DMLHead(emb_module, self.num_classes-1, **self.cls_emb_head_cfg)
    ...
    def forward_single(self, x, scale):
        ...
        cls_score, distance = self.fcos_cls(cls_feat)
        ...
        if self.training:
            return cls_score, bbox_pred, centerness, distance
        else:
            return cls_score, bbox_pred, centerness

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'distances'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
