...
class GARetinaDMLHead19(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        super(GARetinaDMLHead19, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.feature_adaption_cls = FeatureAdaptionCls(
            ...
            self.feat_channels,
            kernel_size=3,
            conv_offset_in_channels=self.deformable_groups_cls,
            conv_offset_kernel_size=3,
    ...
    def calculate_neg_weights(self, labels, label_weights, feature_size):
        ...
        if int(pos_labels.sum()) == 0:
            return torch.full_like(label_weights, 1.).to(label_weights.device)
        pos_xy_list = []
        pos_num_list = []
    ...
    def loss_single(self, cls_score, bbox_pred, labels, label_weights,
        ...
        neg_weights = self.calculate_neg_weights(labels, label_weights, cls_score.shape[-2:]).detach()

        neg_weights = neg_weights.reshape(-1)
        ...
        loss_cls_all_neg_weights = loss_cls_all * neg_weights
        _, neg_selected = loss_cls_all_neg_weights[neg_inds].topk(num_neg_samples)

        # topk_loss_cls_neg, _ = loss_cls_all[neg_inds].topk(num_neg_samples)
        topk_loss_cls_neg = loss_cls_all[neg_selected]
