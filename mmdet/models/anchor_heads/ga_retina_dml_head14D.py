...
class GARetinaDMLHead14D(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        super(GARetinaDMLHead14D, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def forward_single(self, x):
        ...
        # if (emb_vectors.size(-1) < 10):
        #     print('hello!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        #     print(emb_vectors[0, 0, 0, 0])

        distances_bg = torch.sqrt(torch.sum(emb_vectors**2, 1, keepdim=True))
        distances_bg = distances_bg / 8
        probs_bg = torch.exp(-distances_bg**2/(2.0*self.sigma**2))

        emb_vectors_norm = F.normalize(emb_vectors, p=2, dim=1)

        distances = emb_vectors_norm.permute(0, 2, 3, 1).unsqueeze(3).unsqueeze(4)
        ...
        probs = torch.cat((probs_bg, probs_cls_sumj), 1)
        cls_score = probs / probs.sum(1, keepdim=True)
        ...
        if self.training:
            ...
            return cls_score, bbox_pred, shape_pred, loc_pred, distances, distances_bg
        else:
            if self.save_outs:
                return cls_score.log(), bbox_pred, shape_pred, loc_pred, cls_feat, reg_feat, feat_cls, feat_reg, emb_vectors, cls_feat_enhance_pre, probs_bg
            else:
    ...
    def forward(self, feats):
        if self.training:
            ...
            if self.save_outs:
                cls_scores, bbox_preds, shape_preds_reg, loc_preds, cls_feat, reg_feat, cls_feat_adp, reg_feat_adp, emb_vectors, cls_feat_enhance_pres, probs_bg = multi_apply(self.forward_single, feats)
                res = dict()
                ...
                res['probs_bg'] = probs_bg
                save_idx = 1
                save_path_base = 'mytest/ga_retina_dml14D_feature.pth'
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'shape_preds', 'loc_preds', 'distances', 'distances_bg'))
    def loss(self,
             cls_scores,
             bbox_preds,
             shape_preds,
        ...
        # get anchor background embedding loss
        losses_emb_bg = []
        # num_total_samples_emb_bg = sum([int((_==0).sum()) for _ in labels_list])
        num_total_samples_emb_bg = num_total_samples_emb * cfg.neg_pos_ratio
        for i in range(len(distances_bg)):
            loss_emb_bg = self.loss_emb_bg_single(
                distances_bg[i],
                labels_list[i],
                neg_pos_ratio=cfg.neg_pos_ratio)
            losses_emb_bg.append(loss_emb_bg)
        # print('losses_cls: ', losses_cls)
        # print('losses_emb: ', losses_emb)
        # print('losses_emb_bg: ', losses_emb_bg)
        return dict(
            loss_cls=losses_cls,
    ...
    def loss_emb_bg_single(self, distances_bg, labels, neg_pos_ratio):
        labels = labels.reshape(-1)

        pos_inds = (labels > 0).nonzero().view(-1)
        neg_inds = (labels == 0).nonzero().view(-1)

        num_pos_samples = max(int(pos_inds.size(0)), 1)
        num_neg_samples = neg_pos_ratio * num_pos_samples
        if num_neg_samples > neg_inds.size(0):
            num_neg_samples = neg_inds.size(0)

        distances_bg = distances_bg.permute(0, 2, 3, 1).reshape(-1)
        distances_bg_neg = distances_bg[neg_inds]
        loss_emb_bg = F.relu(distances_bg_neg - self.alpha_bg)
        topk_loss_emb_bg, _ = loss_emb_bg.topk(num_neg_samples)
        return topk_loss_emb_bg.mean()

    def loss_emb_single(self, distances, labels, label_weights, num_total_samples):
        # filter out negative samples
        # print(label.size())
        if labels.dim() == 1:
            labels = labels.unsqueeze(0)
            label_weights = label_weights.unsqueeze(0)
        distances = distances.view(distances.size(0), distances.size(1), distances.size(2), -1).permute(0, 3, 1, 2)
        # label_ = label.view(distance.size(0), 1, -1)
        pos_inds = labels > 0

        distance_pos = distances[pos_inds]
        # print(distance_pos.size())
        label_weights_pos = label_weights[labels > 0]
        label_pos = labels[pos_inds].view(-1, 1)
