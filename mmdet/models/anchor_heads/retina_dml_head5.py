...
class RetinaDMLHead5(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
    ...
    def loss_cluster(self, distance, labels, avg_factor):
        pos_inds = labels > 0
        if pos_inds.any() == 0:
            return torch.tensor(0.0).to(labels.device)
        distance_pos = distance.permute(0, 3, 4, 1, 2).flatten(1, 2)[pos_inds].min(2)[0]

        labels_pos = labels[pos_inds]
        labels_pos_one_hot = F.one_hot(labels_pos.flatten(0), num_classes=self.num_classes).byte()[:, 1:]
        distance_pos_target = distance_pos[labels_pos_one_hot]
        loss_cluster = distance_pos_target.sum() / avg_factor
        return loss_cluster
    ...
    def loss(self,
        ...
        for i in range(len(distances)):
            loss_cluster = self.loss_cluster(
                distances[i],
                labels_list[i],
                avg_factor=num_total_samples_emb)
            losses_cluster.append(loss_cluster)

        return dict(loss_cls=losses_cls,
                    loss_bbox=losses_bbox,
                    loss_emb=losses_emb,
                    loss_cluster=losses_cluster
                    )
