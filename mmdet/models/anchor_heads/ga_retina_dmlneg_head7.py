...
class GARetinaDMLNegHead7(GuidedAnchorHead):
    ...
    def forward_single(self, x):
        ...
        if self.save_outs:
            feat_cls, offset_cls = self.feature_adaption_cls(cls_feat, cls_feat, self.save_outs)
            feat_reg, offset_reg = self.feature_adaption_reg(reg_feat, shape_pred, self.save_outs)
        else:
            feat_cls = self.feature_adaption_cls(cls_feat, cls_feat, self.save_outs)
