...
class DMLHead(nn.Module):
    def __init__(self,
        ...
        self.representations = nn.Embedding(self.output_channels* self.num_modes, self.emb_channels[-1]).weight
    ...
    def forward(self, x, save_outs=False):
        ...
        emb_vectors_ex = emb_vectors[:, None, None, :, :, :].repeat(1, self.output_channels, self.num_modes, 1, 1, 1)
        reps_ex = reps[None, :, :, :, None, None].repeat(emb_vectors.shape[0], 1, 1, 1, emb_vectors.shape[2], emb_vectors.shape[3])
        # distances = (emb_vectors_ex*reps_ex).sum(dim=3)
        cos_sim = F.cosine_similarity(reps_ex, emb_vectors_ex, dim=3)
        # print('(cos_sim-distances).sum(): ', (cos_sim-distances).sum())
        ...
        return inverse_sigmoid(cls_score), distances
