...
class FeatureAdaption(nn.Module):
    ...
    def __init__(self,
                 in_channels,
                 out_channels,
                 conv_offset_in_channels,
                 kernel_size=3,
                 deformable_groups=4):
        super(FeatureAdaption, self).__init__()
        offset_channels = kernel_size * kernel_size * 2
        self.conv_offset = nn.Conv2d(
            conv_offset_in_channels, deformable_groups * offset_channels, 3, 1, 1, bias=False)
    ...
    def forward(self, x, reg_info, save_out=False):
        offset = self.conv_offset(reg_info.detach())
...
class RetinaDMLHead8(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
    ...
    def forward_single(self, x):
        ...
        cls_feat = self.feature_adaption(cls_feat, reg_feat)
