...
class GARetinaDMLHead21(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        self.sigma = sigma
        super(GARetinaDMLHead21, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.spatial_attention_conv = nn.Conv2d(2, 1, 3, stride=1, padding=1)
    ...
    def init_weights(self):
        ...
        normal_init(self.spatial_attention_conv, std=0.01)
    ...
    def forward_single(self, x):
        ...
        feat_cls_avg = torch.mean(feat_cls, dim=1, keepdim=True)
        feat_cls_max, _ = torch.max(feat_cls, dim=1, keepdim=True)
        cls_feat_attention_input = torch.cat([feat_cls_avg, feat_cls_max], dim=1)
        cls_feat_attention_pre = self.spatial_attention_conv(cls_feat_attention_input)
        cls_feat_attention_pre = torch.cat([cls_feat_attention_pre.sigmoid(), loc_pred.sigmoid().detach()], dim=1)

        feat_cls_adp = self.feature_adaption_cls(feat_cls, cls_feat_attention_pre)
        ...
        if self.training:
            reps = self.rep(torch.tensor(1.0).to(feat_reg_adp.device).unsqueeze(0)).squeeze(0)
        ...
        if self.training:
            ...
            if self.save_outs:
                return cls_score.log(), bbox_pred, shape_pred, loc_pred, feat_cls, feat_reg, feat_cls_adp, feat_reg_adp, emb_vectors, cls_feat_enhance_pre
            else:
