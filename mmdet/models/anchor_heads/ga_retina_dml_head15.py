...
class GARetinaDMLHead15(GuidedAnchorHead):
    ...
    def loss(self,
        ...
        # get loc targets
        loc_targets = self.ga_loc_target(
            guided_anchors_list_clone,
            labels_list,
            gt_bboxes)
        ...
        for i in range(len(loc_preds)):
            loss_loc = self.loss_loc(
                loc_preds[i].permute(0, 2, 3, 1).reshape(-1 ,1),
                loc_targets[i].reshape(-1, 1),
                label_weights_list[i].reshape(-1, 1),
    ...
    def ga_loc_target(self, guided_anchors_list, labels_list, gt_bboxes):
        ...
        for lvl in range(num_lvl):
            ...
            pos_inds = labels_list[lvl] > 0
            loc_target = torch.zeros_like(pos_inds, dtype=torch.float).to(labels_list[0].device)
            loc_target[pos_inds] = ious_lvl[pos_inds]
