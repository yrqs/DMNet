...
class GARetinaDMLHead6(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        self.sigma = sigma
        super(GARetinaDMLHead6, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def loss_single(self, cls_score, bbox_pred, labels, label_weights,
        ...
        loss_cls_pos = loss_cls_all[pos_inds].flatten()
        loss_cls_neg = topk_loss_cls_neg.flatten()
        loss_cls = torch.cat([loss_cls_pos, loss_cls_neg])
        pt = torch.exp(-loss_cls)
        loss_cls_focal = 0.25 * torch.sub(1, pt)**2.0 * loss_cls
        loss_cls_final = loss_cls_focal.sum() / num_total_samples
        ...
        return loss_cls_final, loss_bbox
