...
class DMLHead(nn.Module):
    ...
    def forward(self, x, save_outs=False):
        ...
        reps_norm = F.normalize(reps, p=2, dim=2)
        ...
        reps_norm_ex = reps_norm[None, :, :, :, None, None].expand_as(reps_ex)
        emb_vectors_ex = emb_vectors[:, None, None, :, :, :].expand_as(reps_ex)
        emb_vectors_ex = emb_vectors_ex * reps_ex.sigmoid()
...
class RetinaDMLHead6(AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
