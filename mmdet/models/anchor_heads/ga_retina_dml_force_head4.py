...
class DMLForceHead(nn.Module):
    def __init__(self,
        ...
        self.representations = nn.Parameter(
            ...
            requires_grad=False)
        self.force_conv = nn.Conv2d(self.emb_channels[-1]*3, self.emb_channels[-1], 1)

        normal_init(self.rep_fc, std=0.01)
        normal_init(self.force_conv, std=0.01)
    ...
    def forward(self, x, save_outs=False):
        ...
        bs = emb_vectors.shape[0]
        num_cls = self.output_channels
        h = emb_vectors.shape[2]
        w = emb_vectors.shape[3]
        ...
        emb_vectors_repeat = emb_vectors.unsqueeze(1).unsqueeze(1).repeat(
            1, self.output_channels, self.num_modes, 1, 1, 1
        )

        r = emb_vectors_repeat - reps_repeat

        emb_rep_r = torch.cat([emb_vectors_repeat, reps_repeat, r], dim=3).reshape(bs*num_cls, self.emb_channels[-1]*3, h, w)
        offsets = self.force_conv(emb_rep_r).reshape(bs, num_cls, self.emb_channels[-1], h, w)
        offsets_sum = offsets.sum(1)

        offset_emb_vectors = emb_vectors + offsets_sum
        offset_emb_vectors = F.normalize(offset_emb_vectors, p=2, dim=1)
        offset_emb_vectors_repeat = offset_emb_vectors.unsqueeze(1).unsqueeze(1).repeat(
            1, num_cls, self.num_modes, 1, 1, 1
        )
        distances_ref = torch.sqrt(((offset_emb_vectors_repeat - reps_repeat)**2).sum(3))
        distances_ori = torch.sqrt(((emb_vectors_repeat - reps_repeat)**2).sum(3))

        cls_score = torch.exp(-(distances_ref)**2/(2.0*self.sigma**2)).squeeze(2)
        cls_score_ori = torch.exp(-(distances_ori)**2/(2.0*self.sigma**2)).squeeze(2)

        if save_outs:
            return cls_score, distances_ref, emb_vectors, reps
        return cls_score, distances_ref, cls_score_ori, distances_ori
...
class GARetinaDMLForceHead4(GuidedAnchorHead):
    ...
    def forward_single(self, x):
        ...
        cls_score, distance, cls_score_ori, distance_ori = self.cls_head(feat_cls)

        cls_score = inverse_sigmoid(cls_score)

        if self.training:
            return cls_score, bbox_pred, shape_pred, loc_pred, distance, cls_score_ori, distance_ori
        else:
            if self.save_outs:
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'shape_preds', 'loc_preds', 'distances', 'cls_scores_ori', 'distances_ori'))
    def loss(self,
             cls_scores,
             bbox_preds,
             shape_preds,
        ...
        for i in range(len(distances)):
            loss_hp_sum, loss_hn_sum, hp_num, hn_num = self.loss_force_single(
                labels_one_hot_list[i],
                cls_scores_ori[i],
                distances_ori[i],
                distances[i])
            losses_force_hp.append(loss_hp_sum)
            losses_force_hn.append(loss_hn_sum)
            num_total_hp += hp_num
            num_total_hn += hn_num
        if num_total_hp > 0:
            losses_force_hp = [l / num_total_hp for l in losses_force_hp]
        else:
            losses_force_hp = [cls_scores_ori[0].new_full((1,), 0.)]
        if num_total_hn > 0:
            losses_force_hn = [l / num_total_hn for l in losses_force_hn]
        else:
            losses_force_hn = [cls_scores_ori[0].new_full((1,), 0.)]
        ...
        return dict(
            ...
            loss_loc=losses_loc,
            loss_emb=losses_emb,
            loss_force_hp=losses_force_hp,
            loss_force_hn=losses_force_hn,
        )
    ...
    def loss_force_single(self, labels_ont_hot, cls_score, distance_ori, distance_ref):
        distance_ori = distance_ori.squeeze(2).permute(0, 2, 3, 1).reshape(labels_ont_hot.shape)
        distance_ref = distance_ref.squeeze(2).permute(0, 2, 3, 1).reshape(labels_ont_hot.shape)
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(labels_ont_hot.shape)
        hp_inds = (labels_ont_hot==1) & (cls_score<self.pos_thresh)
        hn_inds = (labels_ont_hot==0) & (cls_score>self.neg_thresh)
        # print('cls_score: ', (cls_score>self.neg_thresh).sum())
        # print('labels_ont_hot0: ', (labels_ont_hot==0).sum())
        # print('labels_ont_hot1: ', (labels_ont_hot==1).sum())
        # print('hn_inds: ', hn_inds.sum())
        loss_hp = F.relu(distance_ref[hp_inds] - distance_ori[hp_inds] + self.force_alpha)
        loss_hn = F.relu(distance_ori[hn_inds] - distance_ref[hn_inds] + self.force_alpha)
        loss_hp_gz = loss_hp[loss_hp > 0]
        loss_hn_gz = loss_hn[loss_hn > 0]
        hp_num = loss_hp_gz.size(0)
        hn_num = loss_hn_gz.size(0)
        loss_hp_sum = loss_hp_gz.sum()
        loss_hn_sum = loss_hn_gz.sum()
        # print('loss_hn_sum: ', loss_hn_sum)
        return loss_hp_sum, loss_hn_sum, hp_num, hn_num
