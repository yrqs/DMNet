...
class DMLForceHead(nn.Module):
    ...
    def forward(self, x, save_outs=False):
        ...
        offsets = F.relu(self.force_conv(emb_rep_r)).reshape(bs, num_cls, self.emb_channels[-1], h, w)
        ...
        distances = torch.sqrt(((offset_emb_vectors_repeat - reps_repeat)**2).sum(3))
...
class GARetinaDMLForceHead2(GuidedAnchorHead):
