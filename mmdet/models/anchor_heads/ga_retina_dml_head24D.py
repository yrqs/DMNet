...
class GARetinaDMLHead24D(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        self.sigma = sigma
        super(GARetinaDMLHead24D, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.cls_feat_enhance = nn.Sequential(
            nn.Conv2d(self.feat_channels, self.feat_channels//16, 3, stride=1, padding=1),
            nn.Conv2d(self.feat_channels//16, 1, 3, stride=1, padding=1),
        )

        self.reg_feat_enhance = nn.Sequential(
            nn.Conv2d(self.feat_channels, self.feat_channels//16, 3, stride=1, padding=1),
            nn.Conv2d(self.feat_channels//16, 4, 3, stride=1, padding=1),
        )

        self.feature_adaption_cls = FeatureAdaption1(
            self.feat_channels,
            self.feat_channels,
            kernel_size=3,
            conv_offset_in_channels=1,
            conv_offset_kernel_size=3,
            deformable_groups=self.deformable_groups)

        self.feature_adaption_reg = FeatureAdaption1(
            self.feat_channels,
            self.feat_channels,
            kernel_size=3,
            conv_offset_in_channels=4,
            conv_offset_kernel_size=3,
            deformable_groups=self.deformable_groups)
    ...
    def forward_single(self, x):
        ...
        feat_reg = self.feature_adaption_reg(reg_feat, reg_feat_enhance_pre)
        ...
        if self.training:
            ...
            if self.save_outs:
                return cls_score.log(), bbox_pred, shape_pred, loc_pred, cls_feat, reg_feat, feat_cls, feat_reg, emb_vectors
            else:
