...
class GADMLRPN7Head(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        super(GADMLRPN7Head, self).__init__(num_classes, in_channels, **kwargs)
        self.loss_emb = build_loss(loss_emb)
    def _init_layers(self):
        self.relu = nn.ReLU(inplace=True)
        # self.rpn_conv = nn.Conv2d(self.in_channels, self.feat_channels, 3, padding=1)
        # self.emb_conv = nn.Conv2d(self.in_channels, self.feat_channels, 3, padding=1)
        ...
        self.relu = nn.ReLU(inplace=True)
        self.emb_weights = nn.Parameter(torch.FloatTensor(1, 1, 1, 1, 1, self.emb_sizes[-1]))
    ...
    def init_weights(self):
        ...
        # normal_init(self.emb_conv, std=0.01)
        for m in self.emb:
        ...
        constant_init(self.emb_weights, 2.0)
    ...
    def forward_single(self, x):
        ...
        # feat_cls = self.emb_conv(x)
        # feat_reg = self.rpn_conv(x)
        #
        # loc_pred = self.conv_loc(feat_cls)
        # shape_pred_cls = self.conv_shape_cls(feat_cls)
        # shape_pred_reg = self.conv_shape_reg(feat_reg)
        ...
        if self.training:
            dropout_weights = (torch.rand(self.emb_weights.size()) <= 0.9).float().to(feat_reg.device)
            ew = self.emb_weights.sigmoid()*dropout_weights
        else:
            ew = self.emb_weights.sigmoid()

        distances = torch.sqrt(((((distances - reps)**2)*ew).sum(-1))).permute(0, 3, 4, 1, 2).contiguous()
        ...
        # cls_score = probs_fg
        # print(probs_fg)
        probs_bg = -(probs_cls.max(1)[0].max(1, keepdim=True)[0]-1)
    ...
    def loss(self,
        ...
        label_channels = self.cls_out_channels if self.use_sigmoid_cls else 5
        ...
        if not self.finetuning:
            ...
            for i in range(len(distances)):
                loss_emb = self.loss_emb_single(
                    ...
                    num_total_samples=num_total_samples_reg)
