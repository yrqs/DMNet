...
class FeatureEnhance(nn.Module):
    ...
    def __init__(self,
        ...
        self.enhance_conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)

    def init_weights(self):
        normal_init(self.enhance_conv, std=0.1)

    def forward(self, x):
        x_enhance = self.enhance_conv(x)
        x_enhance_pool = self.pool(x_enhance)
        x_enhance = torch.where(x_enhance==x_enhance_pool, x_enhance, torch.full_like(x_enhance, 0))
        return x_enhance


@HEADS.register_module
class GARetinaDMLHead25(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        self.sigma = sigma
        super(GARetinaDMLHead25, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.cls_feat_enhance = FeatureEnhance(self.feat_channels, self.deformable_groups)
    ...
    def init_weights(self):
        ...
        self.cls_feat_enhance.init_weights()
