...
class GARetinaDMLHead3D(GuidedAnchorHead):
    ...
    def __init__(self,
        ...
        super(GARetinaDMLHead3D, self).__init__(num_classes, in_channels, **kwargs)
        self.loss_emb = build_loss(loss_emb)
        self.save_outs = save_outs

        if freeze:
            for c in [self.emb, self.cls_convs, self.reg_convs, self.conv_loc, self.conv_shape,
                      self.cls_feat_enhance, self.feature_adaption_cls, self.feature_adaption_reg,
                      self.retina_reg]:
                for p in c.parameters():
                    p.requires_grad = False
        if finetune:
            for p in self.rep.parameters():
                p.requires_grad = False
    ...
    def forward_single(self, x):
        ...
        cls_feat_enhance_pre = self.cls_feat_enhance(feat_cls)
        # cls_feat_enhance_pre = torch.cat([cls_feat_enhance_pre, loc_pred.detach()], dim=1)

        feat_cls_adp = self.feature_adaption_cls(feat_cls, cls_feat_enhance_pre)
        ...
        if self.training:
            ...
            if self.save_outs:
                return cls_score.log(), bbox_pred, shape_pred, loc_pred, feat_cls, feat_reg, feat_cls_adp, feat_reg_adp, emb_vectors, cls_feat_enhance_pre, probs_bg
            else:
    ...
    def forward(self, feats):
        if self.training:
            ...
            if self.save_outs:
                ...
                save_idx = 1
                save_path_base = 'mytest/ga_retina_dml3D_feature.pth'
